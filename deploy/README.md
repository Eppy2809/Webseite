...
Confirm a separate key-based SSH session, install it as
`/etc/ssh/sshd_config.d/00-valtro-hardening.conf`, run `sshd -t`, and reload instead of
restarting SSH. The `00-` prefix ensures these first-match settings take precedence over
Ubuntu's cloud-init SSH configuration. `valtro-journal-retention.conf` limits system
journal retention to 30 days and 500 MB. Hostinger's external firewall should expose only
TCP 22, TCP 80, TCP 443 and UDP 443, followed by a deny-all rule.
